In the last 6 weeks, select the days when you had sex
...
Qualtrics.SurveyEngine.addOnload(function()
{
 ...
 var y =QBuilder('div');
  ...
  c.appendChild(d);
  var cal1 =new YAHOO.widget.Calendar(calid, { MULTI_SELECT: true, mindate: sixweeksago, maxdate: today }); 
  cal1.render();
  var input = $('QR~'+ qid);
...
cal1.deselectEvent.subscribe(function(e,dates){
  ...
  input.value = resp;
	  ...
	  var responseTextField = document.getElementById(qid + '~1~result');
	  console.log(responseTextField);
	  var currentResponse = responseTextField.value;
	  console.log(currentResponse);
	  alert("Result: " + currentResponse + "\nwill be available to future questions as: \n$" + "{e://Field/" + 'ssdates' + "}");
	  Qualtrics.SurveyEngine.setEmbeddedData( 'ssdates', currentResponse );
